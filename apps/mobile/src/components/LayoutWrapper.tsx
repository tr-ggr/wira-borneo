...
export default function LayoutWrapper({
  ...
  return (
    <div className="flex flex-col h-screen bg-wira-ivory wira-batik-bg overflow-hidden">
      ...
      <main className={`flex-1 ${showNav ? 'pb-20' : 'pb-4'} px-4 pt-4 overflow-y-auto w-full max-w-md mx-auto scroll-smooth`}>
        {children}
      </main>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[9px] font-body font-bold uppercase tracking-tighter ${isActive ? 'opacity-100' : ''}`}>{item.label}</span>
              </button>
            );
          })}
        </nav>
