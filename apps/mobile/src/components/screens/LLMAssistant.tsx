...
export default function LLMAssistant() {
  ...
  const inquire = useAssistantControllerInquire({
    mutation: {
      onSuccess: (data: any) => {
        setMessages(prev => [...prev, { role: 'assistant', content: data.response || 'Sorry, I cannot process that information right now.' }]);
      },
      onError: () => {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setInput('');
    inquire.mutate({ data: { query: userMessage } });
  };
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}>
            <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm font-body leading-relaxed shadow-sm ${
              msg.role === 'user' 
      ...
      <div className="px-4 py-4 shrink-0 bg-white">
        <form onSubmit={handleSend} className="relative">
          ...
          <button 
            ...
            className={`absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 rounded-xl flex items-center justify-center transition-all ${
              input.trim() ? 'bg-wira-gold text-white' : 'bg-wira-ivory-dark text-wira-earth/30'
            }`}
          >
            <Send size={18} />
          </button>
